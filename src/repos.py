        ...
        dirs.append(
            {
                ...
                "name_for_parent": int(
                    os.getenv(f"search_name_for_parent_{i}", "1") or "1"
                ),
            }
        )
    return dirs
...
def search_config_hash(search_dirs, global_excludes):
    ...
    blob = json.dumps(
        {"dirs": search_dirs, "excludes": global_excludes}, sort_keys=True
    )
...
def repo_url(path):
    ...
    remotes = (
        subprocess.check_output(["git", "remote"], cwd=path)
        .decode("utf-8")
        .splitlines()
    )
    ...
    if remote_name not in remotes:
        notify(
            "No remote named {}".format(remote_name),
            "Check your settings",
            "Available remotes: {}".format(", ".join(remotes)),
            "Sosumi",
        )
    ...
    url = subprocess.check_output(
        ["git", "config", "remote." + remote_name + ".url"], cwd=path
    ).decode("utf-8")
...
def _resolve_open_target(mode, custom_path, repo_path):
    ...
    """
    use_url = mode == MODE_BROWSER or (
        mode == MODE_CUSTOM and custom_path and is_browser(custom_path)
    )
...
def do_open(opts):
    ...
    if app is None:
        print(
            "App {} not set. Configure this workflow in Alfred Preferences.".format(
                opts.appkey
            )
        )
        return 0
...
def do_search(repos, opts):
    ...
    needs_refresh = wf.cached_data_age(
        "repos_v2"
    ) > opts.update_interval and not is_running("update")
    if needs_refresh:
        wf.rerun = 2.0
    ...
    if not items:
        wf.add_item(
            "No matching worktrees found" if expanded else "No matching repos found",
            icon=ICON_WARNING,
        )
    ...
    for r in items:
        ...
        wt_hint = (
            " (+{} worktree{})".format(wt_count, "" if wt_count == 1 else "s")
            if (not expanded and wt_count)
            else ""
        )
        branch_info = (
            "{} {}{}  |  ".format(BRANCH_ICON, r.branch, wt_hint) if r.branch else ""
        )
