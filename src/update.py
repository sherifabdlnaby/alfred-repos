    ...
    for filepath in output:
        ...
        if name_for_parent < 2:  # ignore 0, it's pointless
            ...
            if name_for_parent >= len(components):
                log.warning(
                    "%s : `name_for_parent` is %d, but only %d levels in file tree",
                    filepath,
                    name_for_parent,
                    len(components),
                )
        ...
        results.append(
            Repo(name, filepath, get_branch(filepath), enumerate_worktrees(filepath))
        )

    log.debug(
        "%d repo(s) found in `%s` in %0.2fs", len(results), dirpath, time() - start
    )
...
def main(wf):
    ...
    if not search_dirs:
        log.error("No search directories configured. Nothing to update. Exiting.")
        return 0
    ...
    for data in search_dirs:
        ...
        r = pool.apply_async(
            find_git_repos, (dirpath, excludes, depth, uid, gids, name_for_parent)
        )
        results.append(r)
